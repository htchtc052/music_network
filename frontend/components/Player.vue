<script lang="ts" setup>
  ...
  const filePath: ComputedRef<string | null> = computed(() => {
    const newFilePath =
      track.value != null
        ? "http://localhost:4100" + track.value.trackFile?.filePath.replace("uploads", "")
        : null;
    return newFilePath;
  });
  ...
  const updateTime = () => {
    (audioPlayer.value as HTMLAudioElement).currentTime = progressBar.value.value | 0;
  };
  ...
  watch(filePath, (newValue) => {
    ...
    if (audioPlayer.value) {
      ...
      audioPlayer.value.addEventListener("loadedmetadata", loadedMetadateListener);
    ...
    <div class="flex items-center">
      ...
      <span class="next_btn" title="Next" @click="nextClick()">
        <svg
          ...
          xmlns="http://www.w3.org/2000/svg"
        >
          <path d="M9 5l7 7-7 7" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2px" />
        </svg>
      </span>

      <div class="mr-3 flex-grow">
        <div class="truncate text-base font-bold leading-5">!! {{ track?.title }}!!</div>
      </div>
      ...
      <input ref="volume" max="1" min="0" step="0.01" type="range" @input="setVolume" />
    </div>
  </div>
</template>
<style></style>
